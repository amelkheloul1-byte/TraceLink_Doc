...
void MoteurTracabilite::analyserSSS_SRS() {
    for (const auto& srs : exigencesSRS) {
        for (const auto& ref : srs.getTracabilite()) {
            std::cout << "[DEBUG MATCH] ref='" << ref << "'" << std::endl;
            bool trouve = false;
            ...
            if (!trouve) {
                // chercher le fichier SSS correspondant via necessaireA et alloueA
    ...
    for (const auto& srs : exigencesSRS) {
        if (srs.getTracabilite().empty()) {
            // AJOUT : chercher le fichier SSS correspondant via necessaireA et alloueA
            std::string fichierCible = "SSS";
            for (const auto& sss : exigencesSSS) {
                const auto& alloue = sss.getAlloueA();
                const auto& necSRS = srs.getNecessaireA();
            ...
            orphelines.push_back(
                ...
                " ne reference aucune exigence SSS dans " + fichierCible
                );
        }
    }
    for (auto& sss : exigencesSSS) {
...
void MoteurTracabilite::analyserSRS_SDD() {
    ...
    for (const auto& sdd : exigencesSDD) {
        if (sdd.getReferencedSRS().empty()) {
            // AJOUT : pour orpheline SDD, on indique juste "SRS" car SDD n'a pas de necessaireA
            orphelines.push_back(
...
void MoteurTracabilite::calculerTaux() {
    ...
    // Une SSS est complètement tracée si :
    //   1. Elle est couverte par au moins une SRS
    //   2. Cette SRS est elle-même couverte par au moins une SDD
    if (exigencesSSS.empty()) {
        tauxTracabiliteComplete = 0.0;
    } else {
        int nbCompletes = 0;
        for (const auto& sss : exigencesSSS) {
            // Trouver toutes les SRS qui couvrent cette SSS
            ...
            for (const auto& srs : exigencesSRS) {
                ...
                if (srsCouvreSss && srs.getStatut() == Couverte) {
                    // Cette SRS couvre la SSS ET est elle-même couverte par une SDD
                    chaineComplete = true;
                    break;
                }
            }
