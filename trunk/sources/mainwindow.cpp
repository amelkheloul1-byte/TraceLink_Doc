...
{
    // Nettoyage sécurisé du thread si la fenêtre est fermée pendant l'analyse
    if (threadManager) {
 ...
 *        Gauche = sélection fichiers | Milieu = terminal | Droite = aperçu
 */
void MainWindow::construireOngletImport()
{
    ongletImport = new QWidget();
    ongletImport->setStyleSheet("background-color: #EBF5FB;");

    QHBoxLayout *layoutPrincipal = new QHBoxLayout(ongletImport);
    layoutPrincipal->setContentsMargins(16, 16, 16, 16);
    layoutPrincipal->setSpacing(12);

    // ══════════════════════════════════════════
    // COLONNE GAUCHE — sélection fichiers
    // ══════════════════════════════════════════
    QWidget *colonneGauche = new QWidget();
    colonneGauche->setFixedWidth(300);
    colonneGauche->setStyleSheet("background-color: #EBF5FB;");
    ...
    ongletsFichiers->setStyleSheet(R"(
        ...
        QTabBar::tab:selected { background: #2E86C1; color: white; border: none; }
    )");
    ...
    champSSS->setPlaceholderText("Aucun fichier...");
    champSSS->setReadOnly(true);
    btnParcourirSSS = new QPushButton("📁  Parcourir");
    badgeSSS = new QLabel("");
    badgeSSS->setAlignment(Qt::AlignCenter);
    badgeSSS->setTextFormat(Qt::RichText);
    erreurSSS = new QLabel("");
    erreurSSS->setStyleSheet("color: #E74C3C; font-size: 9px;");
    erreurSSS->setWordWrap(true);
    lSSS->addWidget(champSSS);
    lSSS->addWidget(btnParcourirSSS);
    lSSS->addWidget(badgeSSS);
    lSSS->addWidget(erreurSSS);
    lSSS->addStretch();
    ...
    champSRS->setPlaceholderText("Aucun fichier...");
    champSRS->setReadOnly(true);
    btnParcourirSRS = new QPushButton("📁  Parcourir");
    badgeSRS = new QLabel("");
    badgeSRS->setAlignment(Qt::AlignCenter);
    badgeSRS->setTextFormat(Qt::RichText);
    erreurSRS = new QLabel("");
    erreurSRS->setStyleSheet("color: #E74C3C; font-size: 9px;");
    erreurSRS->setWordWrap(true);
    lSRS->addWidget(champSRS);
    lSRS->addWidget(btnParcourirSRS);
    lSRS->addWidget(badgeSRS);
    lSRS->addWidget(erreurSRS);
    lSRS->addStretch();
    ...
    champSDD->setPlaceholderText("Aucun fichier...");
    champSDD->setReadOnly(true);
    btnParcourirSDD = new QPushButton("📁  Parcourir");
    badgeSDD = new QLabel("");
    badgeSDD->setAlignment(Qt::AlignCenter);
    badgeSDD->setTextFormat(Qt::RichText);
    erreurSDD = new QLabel("");
    erreurSDD->setStyleSheet("color: #E74C3C; font-size: 9px;");
    erreurSDD->setWordWrap(true);
    lSDD->addWidget(champSDD);
    lSDD->addWidget(btnParcourirSDD);
    lSDD->addWidget(badgeSDD);
    lSDD->addWidget(erreurSDD);
    lSDD->addStretch();
    ...
    labelStatut->setStyleSheet(R"(
        color: #888888; font-size: 10px; padding: 6px;
        background-color: #FFFFFF; border-radius: 6px; border: 1px solid #DDDDDD;
    )");
    ...
    btnLancer->setStyleSheet(R"(
        QPushButton {
            ...
            border-radius: 8px; font-size: 12px; font-weight: bold; border: none;
        }
        QPushButton:!disabled { background-color: #27AE60; color: white; border: none; }
        QPushButton:!disabled:hover { background-color: #1E8449; }
    )");
    ...
    btnReinitialiser->setStyleSheet(R"(
        QPushButton {
            ...
            border-radius: 6px; font-size: 11px; border: 1px solid #DDDDDD;
        }
        QPushButton:hover { background-color: #FDECEA; color: #E74C3C; border-color: #E74C3C; }
    )");
    ...
    colonneMilieu->setStyleSheet("background-color: #EBF5FB;");
    QVBoxLayout *layoutMilieu = new QVBoxLayout(colonneMilieu);
    layoutMilieu->setContentsMargins(0, 0, 0, 0);
    layoutMilieu->setSpacing(10);

    QLabel *titreTerminal = new QLabel("Journal d'analyse");
    titreTerminal->setStyleSheet("font-size: 11px; font-weight: bold; color: #333333;");
    ...
    titreProgress->setStyleSheet("font-size: 11px; font-weight: bold; color: #333333;");
    ...
    barreProgression->setFixedHeight(18);

    layoutMilieu->addWidget(titreTerminal);
    ...
    colonneDroite->setFixedWidth(260);
    colonneDroite->setStyleSheet("background-color: #EBF5FB;");
    ...
    titreApercu->setStyleSheet("font-size: 11px; font-weight: bold; color: #333333;");
    ...
    labelNbExigences->setStyleSheet("color: #888888; font-size: 10px;");

    layoutDroite->addWidget(titreApercu);
...
/**
 * @brief Construit l'onglet Graphe.
 */
...
/**
 * @brief Construit l'onglet Rapport.
 */
...
/**
 * @brief Met à jour le titre et couleur de l'onglet SSS/SRS/SDD.
 * @param index 0=SSS, 1=SRS, 2=SDD
 * @param etat "accepte" | "erreur" | "attente"
 */
...
void MainWindow::parcourirSSS()
{
    QStringList chemins = QFileDialog::getOpenFileNames(
        ...
        "Fichiers supportés (*.docx *.csv *.xls *.txt);;Tous (*)"
        );
    if (!chemins.isEmpty()) {
        cheminSSS = chemins.first();
        champSSS->setText(chemins.join(" | "));
        if (formatAccepte(cheminSSS)) {
            badgeSSS->setText("<span style='color:#27AE60; font-size:18px;'>✓ Fichier valide</span>");
            erreurSSS->setText("");
            mettreAJourOngletFichier(0, "accepte");
        } else {
            cheminSSS = "";
            badgeSSS->setText("<span style='color:#E74C3C; font-size:18px;'>✗ Format invalide</span>");
            erreurSSS->setText("⚠ Acceptés : .docx  .csv  .xls");
...
void MainWindow::parcourirSRS()
{
    ...
    if (!chemins.isEmpty()) {
        cheminSRS = chemins.first();
        champSRS->setText(chemins.join(" | "));
        if (formatAccepte(cheminSRS)) {
            badgeSRS->setText("<span style='color:#27AE60; font-size:18px;'>✓ Fichier valide</span>");
            erreurSRS->setText("");
            mettreAJourOngletFichier(1, "accepte");
        } else {
            cheminSRS = "";
            badgeSRS->setText("<span style='color:#E74C3C; font-size:18px;'>✗ Format invalide</span>");
            erreurSRS->setText("⚠ Acceptés : .docx  .csv  .xls");
...
void MainWindow::parcourirSDD()
{
    ...
    if (!chemins.isEmpty()) {
        cheminSDD = chemins.first();
        champSDD->setText(chemins.join(" | "));
        if (formatAccepte(cheminSDD)) {
            badgeSDD->setText("<span style='color:#27AE60; font-size:18px;'>✓ Fichier valide</span>");
            erreurSDD->setText("");
            mettreAJourOngletFichier(2, "accepte");
        } else {
            cheminSDD = "";
            badgeSDD->setText("<span style='color:#E74C3C; font-size:18px;'>✗ Format invalide</span>");
            erreurSDD->setText("⚠ Acceptés : .docx  .csv  .xls");
...
void MainWindow::verifierFichiersSelectionnes()
{
    ...
    if (pret) {
        ...
        labelStatut->setStyleSheet(R"(
            color: #27AE60; font-size: 10px; padding: 6px;
            background-color: #F0FFF4; border-radius: 6px; border: 1px solid #27AE60;
        )");
    } else {
...
/**
 * @brief Vide toutes les zones et remet à zéro.
 */
...
/**
 * @brief Ajoute un message coloré dans le terminal.
...
{
    terminal->append(
        QString("<span style='color:%1;'>%2</span>").arg(couleur, message)
        );
}
                                         ...
                                         const std::vector<ExigenceExtraite> &sdd)
{
    ...
    for (const auto &e : sss)
        toutesExigences.append("[SSS] " + QString::fromStdString(e.identifiant));
    for (const auto &e : srs)
        toutesExigences.append("[SRS] " + QString::fromStdString(e.identifiant));
    for (const auto &e : sdd)
        toutesExigences.append("[SDD] " + QString::fromStdString(e.identifiant));
 ...
 * L'extraction (M1) se fait dans le thread principal.
 * L'analyse de traçabilité (M2) se fait dans un thread séparé via ThreadManager.
 * Un QTimer surveille la fin du thread toutes les 100ms.
 */
void MainWindow::lancerAnalyse()
{
    try {
        ...
        std::string motif = "[A-Z][A-Z0-9_]+";

        // ── Module 1 : extraction (thread principal) ──
        logTerminal("📂 Lecture SSS : " + cheminSSS, "#888888");
        ExtracteurFichier extracteurSSS(cheminSSS.toStdString(), motif);
        auto extraitesSSS = extracteurSSS.extraire();
        logTerminal("✅ " + QString::number(extraitesSSS.size()) + " exigences SSS détectées", "#27AE60");
        barreProgression->setValue(20);

        logTerminal("📂 Lecture SRS : " + cheminSRS, "#888888");
        ExtracteurFichier extracteurSRS(cheminSRS.toStdString(), motif);
        auto extraitesSRS = extracteurSRS.extraire();
        logTerminal("✅ " + QString::number(extraitesSRS.size()) + " exigences SRS détectées", "#27AE60");
        barreProgression->setValue(40);

        std::vector<ExigenceExtraite> extraitesSDD;
        if (!cheminSDD.isEmpty()) {
            logTerminal("📂 Lecture SDD : " + cheminSDD, "#888888");
            ExtracteurFichier extracteurSDD(cheminSDD.toStdString(), motif);
            extraitesSDD = extracteurSDD.extraire();
            logTerminal("✅ " + QString::number(extraitesSDD.size()) + " exigences SDD détectées", "#27AE60");
        }
        ...
        // QTimer surveille la fin du thread toutes les 100ms
        ...
        connect(timer, &QTimer::timeout, this, [this, timer, srs, sdd]() {
            if (threadManager->estTerminee()) {
                ...
                // DEBUG
                for (const auto& lb : rapport.liensBrises)
                ...
                auto srsAnalyses = moteurAnalyse->getExigencesSRS();
                auto sddAnalyses = moteurAnalyse->getExigencesSDD();
                ongletRapport->afficherRapportComplet(rapport, srsAnalyses, sddAnalyses);
                ongletGraphe->construireGraphe(rapport, srsAnalyses, sddAnalyses);
                ...
                // Animation barre pendant l'attente
