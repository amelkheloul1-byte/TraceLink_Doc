...
std::vector<std::pair<int, std::string>> ExtracteurFichier::lireXlsx() const {
    std::vector<std::pair<int, std::string>> lignes;
    int erreur = 0;
    bool premiereLingeIgnoree = false; // ignorer l'en-tête
    ...
    for (pugi::xpath_node row_xpath : rows) {
        ...
        if (!texteLigne.empty()) {
            numLigne++;
            if (!premiereLingeIgnoree) {
                premiereLingeIgnoree = true;
            } else {
    ...
    const std::string& format) const
{
    ...
    for (size_t i = 0; i < lignes.size(); i++) {
        ...
        if (colonnes.size() >= 2) {
            ...
            std::smatch m;
            // MODIF : accepte tout ID matchant la regex, pas juste EXIGENCE_ ou SDD_
            if (!col2.empty() && std::regex_search(col2, m, reg) && col2 != idPrincipal) {
                ex.tracabilite.push_back(m.str());
            }
        }

        // lecture NecessaireA + Developpe + Description
        if (colonnes.size() >= 5) {
            ex.necessaireA.push_back(colonnes[2]);
            std::string upper4 = colonnes[3];
            std::transform(upper4.begin(), upper4.end(), upper4.begin(), ::toupper);
            // dans CSV developpe est 1 ou 0
    ...
    for (const auto& para : paragraphes) {
        ...
        if (dansBloc) {
            if (texte.find("Tra") != std::string::npos
                ...
                if (exCourante.description.empty()) {
