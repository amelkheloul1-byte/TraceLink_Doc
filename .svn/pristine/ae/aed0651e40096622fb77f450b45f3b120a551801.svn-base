 ...
 *
 * FUSION : version originale (Lamia) + ameliorations export HTML et PDF (Amel Kheloul).
 * Modification v2.1 par Amel Kheloul :
 *   exporterHTML : refonte en tableau 3 colonnes interactif avec recherche et filtres JS
 *   exporterPDF  : creation initiale avec en-tete, police 8pt, format A4 paysage
 */
...
void OngletRapport::appliquerFiltre(const FiltreGUI& filtre)
{
    ...
    for (int i = 0; i < tableMatriceSRS_SDD->rowCount(); i++) {
        ...
        // Une SRS est orpheline si sa liste de tracabilite parents SSS est vide
 ...
 * et la liste des liens brises si presents.
...
void OngletRapport::exporterCSV()
{
    ...
    msg.setText("CSV exporte avec succes !");
 ...
 *   Filtres rapides : Tout, Couvert, Non couvert, Lien brise, Orpheline
...
void OngletRapport::exporterHTML()
{
    ...
    // Lambda : retourne la couleur selon le taux (vert >= 80, orange >= 50, rouge sinon)
    ...
    // Lambda local : genere une carte taux avec barre de progression animee
    ...
    msg.setText("HTML exporte avec succes ! Ouvrez le fichier dans un navigateur.");
    msg.setStyleSheet("color: black; background-color: white;");
    msg.exec();
}
 ...
 *
 * Format A4 paysage, police Arial 8pt, cellules compactes.
 * Contenu : en-tete avec titre et date, matrice SSS x SRS avec statuts colores.
 */
void OngletRapport::exporterPDF()
{
    ...
    QString html = "<html><head><style>"
                   ...
                   ".entete { background-color: #EBF5FB; border: 1px solid #AED6F1; border-radius: 4px; padding: 8px 12px; margin-bottom: 10px; }"
                   ".entete-titre { font-size: 13pt; font-weight: bold; color: #1A5276; }"
                   ".entete-sous  { font-size: 8pt; color: #555; margin-top: 2px; }"
    ...
    // En-tete avec titre et date de generation
    html += "<div class=\"entete\">";
    html += "<div class=\"entete-titre\">TraceLink - Rapport de tracabilite</div>";
    html += "<div class=\"entete-sous\">Projet L2G1 - Universite Paris Cite - Genere le : "
            + QDateTime::currentDateTime().toString("dd/MM/yyyy HH:mm") + "</div>";
    html += "</div>";

    // Matrice SSS x SRS avec statuts colores
    ...
    // Configuration QPrinter : A4 paysage, haute resolution, marges 12mm
    ...
    // Impression via QTextDocument avec police Arial 8pt
    QTextDocument doc;
    ...
    msg.setText("PDF exporte avec succes !");
    msg.setStyleSheet("color: black; background-color: white;");
    msg.exec();
}