...
void GestionnaireFiltresGUI::initialiserUI()
{
    ...
    comboStatut->addItem("🟣  Orpheline",          99); // AJOUT
    comboStatut->setStyleSheet(styleCombo);
    layout->addWidget(comboStatut);
    layout->addSpacing(4);

    // ── État développé ────────────────────────────────────────────────────────
    layout->addWidget(creerLabel("État"));
    ...
    // Conteneur avec icône de recherche
    ...
    // MODIF : autocomplétion avec MatchContains pour recherche par mots-clés
    completer = new QCompleter(listeIds, this);
    completer->setCaseSensitivity(Qt::CaseInsensitive);
    completer->setFilterMode(Qt::MatchContains); // MODIF : contient au lieu de commence par
...
void GestionnaireFiltresGUI::appliquerFiltres()
{
    ...
    filtreCourant.filtreStatut    = static_cast<Statut>(statut == 99 ? NonAnalyse : statut);
...
// MODIF : recherche par mots-clés (contains) au lieu de startsWith
